...
CLASS lhc_rating IMPLEMENTATION.
  METHOD DetermineRatingDate.
    DATA ratings TYPE TABLE FOR UPDATE zr_09_ratingTP.
    ...
    MODIFY ENTITIES OF ZR_09_MovieTP IN LOCAL MODE
           ENTITY rating
           UPDATE
           FIELDS ( RatingDate )
           WITH ratings.
...
CLASS lhc_Movie IMPLEMENTATION.
  ...
  METHOD ValidateGenre.
    DATA movies TYPE TABLE FOR READ RESULT ZR_09_MovieTP.

    READ ENTITIES OF ZR_09_MovieTP IN LOCAL MODE
         ENTITY movie
         FIELDS ( Genre )
    ...
    LOOP AT movies INTO DATA(movie).
      ...
      SELECT SINGLE FROM ZI_09_GenreVH
        FIELDS @abap_true
  ...
  ENDMETHOD.
    ...
    DATA ratings TYPE TABLE FOR CREATE ZR_09_MovieTP\_Ratings.
